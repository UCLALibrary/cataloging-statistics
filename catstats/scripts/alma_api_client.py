...
class Alma_Api_Client():
	...
	def _call_post_api(self, api, data, parameters={}):
		...
		response = requests.post(post_url, headers=self.HEADERS, json=data, params=parameters)
		# Actually, a Python dictionary...
		api_data = response.json()
		# Add a few response elements caller can use
		api_data['api_response'] = {
	...
	def wait_for_completion(self, job_id, instance_id, seconds_to_poll=15):
		...
		# This method allows the caller to wait until the given instance of the job has completed.
		...
		while status in ['NONE', 'QUEUED', 'PENDING', 'INITIALIZING', 'RUNNING', 'FINALIZING']:
	...
	def get_analytics_report(self, parameters={}):
		# Docs say to URL-encode report name (path); they lie, or request lib is doing it automatically?
