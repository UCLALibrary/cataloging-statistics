...
def get_filter(yyyymm):
    ...
    # >
    # 	<sawx:expr xsi:type="sawx:sqlExpression">LOWER("Bibliographic Details"."Local Param 02")
    # </sawx:expr>
...
def run_report(api_key, yyyymm):
    ...
    report_path = (
        ...
        "Cataloging/Reports/API/Cataloging Statistics (API)"
    )
    # From form
    # yyyymm = '202101'
    ...
    merged_params = constant_params | initial_params
    try:
        report = alma_client.get_report(merged_params)
    except APIError as api_ex:
        logger.error(f"APIError on first batch: {api_ex}")
        raise
    ...
    while report_data["is_finished"] == "false":
        ...
        merged_params = constant_params | subsequent_params
        try:
            report = alma_client.get_report(merged_params)
            report_data = get_report_data(report)
            all_rows.extend(report_data["rows"])
        except APIError as api_ex:
            logger.error(f"APIError on subsequent batch: {api_ex}")
            raise
...
def row_is_wanted(row, filters):
    ...
    # Cataloger initials in $b: Filter may be an empty string
        ...
        filters["cataloger"] != ""
        and filters["cataloger"] not in f962.get("b", "").lower()
    ):
        is_wanted = False
    ...
    elif filters["f962_k_code"] != "" and filters["f962_k_code"] not in f962.get(
        "k", ""
    ):
        is_wanted = False
    # Non-962 filters
        ...
        and filters["language_code"] != row["Language Code"]
    ):
        is_wanted = False
...
def expand_and_filter_data(report_data, filters=None):
    ...
    for row in rows:
        ...
        for fld_962 in fld_962s:
            ...
            sfd_dict = {
                code: value
                for sfd in fld_962.split("$$")[1:]
                for code, value in [sfd.strip().split(" ", 1)]
            }
